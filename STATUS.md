...
- Authenticated staging is now partially validated on the same non-production host: the dedicated Google account persisted in a fresh browser process; targeted account/catalog/mode run `20260714T223639Z-e71b3e2e` passed 48/48 checks in mobile-common and laptop with zero issues; and the strict `g12-session-start-smoke` reset contract completed twice idempotently. The run verified explicit account switching/logout controls without clicking logout, all three canonical mode values, tutor-route mode continuity, and the approved lesson ID.
