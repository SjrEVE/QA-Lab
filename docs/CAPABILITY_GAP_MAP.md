...
QA Lab is `EDUCATION_EVAL_FIXTURE_READY`, not AI-evaluator, native-voice, or staging-accepted. Phase 7 provides deterministic-first grading, a versioned fractions rubric, scripted evaluator boundary, and integrated artifacts. Native Linux routing, real provider/evaluator, physical microphone, replay/regression, Model Arena, cohorts, and safety/optimization are not claimed.
