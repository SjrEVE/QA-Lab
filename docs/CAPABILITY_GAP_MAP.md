...
| App Check staging enforcement | true | true | true | false | The runner now requires a non-empty `X-Firebase-AppCheck` header on every request to start/resume, advance and submit-answer APIs and stores counts only. Historical evidence used a weaker any-header-or-token-exchange rule, so enforcement acceptance is pending a new staging run. |
| Targeted authenticated session start | true | true | false | false | Run `20260715T081058Z-e7598a7c` reached `setupComplete` and `lessonReady` but failed latency/whiteboard acceptance. Newer run `20260715T100536Z-cceeee98` reached `setupComplete` in 7.441 s, then staging returned `404` for `/api/resumeLessonSession`; QA Brain/TTS was never invoked and no Live acceptance is claimed. |
...
| Student text QA | Implemented; provider adapter locally validated | `src/student-contracts.ts`; `src/student-brain.ts`; `src/gemini-student-brain.ts`; `src/student-qa.ts`; one persona/scenario; positive/negative provider-boundary tests; ignored fixture evidence | Deterministic text-mode MVP and opt-in Gemini StudentBrain adapter have bounded context, strict structured actions, Vietnamese speech validation, secret-safe transport and reset boundary. A real-key doctor run and a product-specific Student QA staging scenario are still required before provider-backed staging acceptance. No real evaluator acceptance. |
...
- **Provider credentials and data policy:** the Gemini StudentBrain uses an operator-supplied process environment key, an explicit opt-in flag, the fixed official host, bounded synthetic context and no raw-provider persistence. No credential is committed. Real-key doctor evidence and provider/privacy review remain required before broader provider acceptance.
