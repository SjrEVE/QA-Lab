...
| Authenticated account/catalog/mode | true | true | true | false | Run `20260714T223639Z-e71b3e2e` passed 48/48 across mobile-common and laptop with zero issues: hashed identity, explicit switch/logout controls, hierarchy, three canonical modes, tutor-route mode continuity, approved runtime lesson ID continuity, start CTA, events, overflow, and overlap. Logout is checked but deliberately not clicked so the verified private profile is preserved. |
