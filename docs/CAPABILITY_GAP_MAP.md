...
| Targeted authenticated session start | true | true | false | false | Visible staging run `20260715T081058Z-e7598a7c` reached `setupComplete`, detected synthetic Vietnamese speech, reached `lessonReady`, observed verifier/tutor responses and then failed: setup was ~10.5 s, onboarding ~57.7 s, tutor first response ~9.4 s/full turn ~20.6 s, and whiteboard object count stayed zero. Provider connectivity is no longer the blocker; complete Live learning/whiteboard/latency acceptance remains unmet. |
