...
QA Lab is `VOICE_BRIDGE_FIXTURE_READY`, not native-voice or staging-accepted. Phase 6 provides vendor-neutral voice contracts, deterministic WAV/fake routing evidence, Linux setup/probe, permission gating, and text fallback. Native Linux routing, real provider, physical microphone, Education Eval, replay/regression, Model Arena, cohorts, and safety/optimization are not claimed.
