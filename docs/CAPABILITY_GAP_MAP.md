...
QA Lab is `REPLAY_REGRESSION_FIXTURE_READY`, not AI-evaluator, native-voice, or staging-accepted. Phase 8 provides redacted unified timelines, deterministic provider-free replay, baseline comparison, issue lifecycle/deduplication, metric deltas, incident packaging, and fixture reports. Native Linux routing, real provider/evaluator, physical microphone acceptance, Model Arena, cohorts, dashboard, staging/production, auto-fix, and safety/optimization are not claimed.
