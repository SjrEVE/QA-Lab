...
QA Lab is `STUDENT_TEXT_QA_MVP_READY`, not staging-accepted. Phase 0–4 provides guarded browser execution, deterministic Web QA, and a browser-only Student text QA MVP proven against a loopback eight-turn lesson fixture. Recording, voice, provider/AI Education Eval, replay/regression, Model Arena, cohorts, and safety/optimization remain future work in authoritative order.
