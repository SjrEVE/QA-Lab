...
The complete Phase 0–10 authority roadmap is implemented at deterministic local-fixture scope. Separately authorized public/auth/catalog/reset and two guided-self-study modules have bounded staging evidence. An opt-in Gemini StudentBrain adapter is implemented but provider acceptance remains pending. Native Linux voice/recording, physical microphone, independent provider evaluator, production, public commerce, real-child use, auto-fix, provider mutation and deployment remain unaccepted, blocked or forbidden by their existing gates.
