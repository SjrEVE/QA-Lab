...
export const STUDENT_BEHAVIORS = ['silence_once', 'off_topic_once', 'repeat_mistake_once', 'ask_for_example_once'] as const;
export const STUDENT_SCENARIO_GOALS = ['misconception_detected', 'explanation_changes', 'independent_check', 'independent_success'] as const;
...
export const studentPersonaSchema = z.object({
  ...
  behaviors: z.array(z.enum(STUDENT_BEHAVIORS)).min(1).max(8).refine((v) => new Set(v).size === v.length, 'behaviors must be unique'),
}).strict();
...
export const studentScenarioSchema = z.object({
  ...
  goals: z.array(z.enum(STUDENT_SCENARIO_GOALS)).min(1).max(8).refine((v) => new Set(v).size === v.length, 'goals must be unique'),
