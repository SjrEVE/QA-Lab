...
function scenario() { return guidedSelfStudyScenarioSchema.parse({
  ...
  answers: exerciseIds.map((exerciseId, index) => ({ exerciseId, value: values[index], ...(index === 0 ? { incorrectValue: '3' } : {}) })), limits: { maxMinutes: 2, selectorTimeoutMs: 5_000, transitionTimeoutMs: 5_000, maxIssues: 20 },
}); }
